...
class MusicPlayer:
    def __init__(self):
        ...
        self.ydl_opts = {
            ...
            'outtmpl': os.path.join(self.download_dir, 'song.%(ext)s'),  # Save to downloads/
            'quiet': True,
            'no_warnings': True,
        ...
        with self.queue_lock:
            ...
            for i in range(len(self.queue) - 1, -1, -1):
                ...
                if requested_by == username:
                    ...
                    # Clean up pre-downloaded file if it was the first song
                    if i == 0:
                        self.cleanup_pre_downloaded_files()
                        self.pre_download_first_song()  # Download new first song
    ...
    def play_next(self, user=None):
        ...
        with self.queue_lock:
            if not self.queue:
                ...
                self.cleanup_root_files()  # Clean up stray files in root
        ...
        def play_in_thread():
            try:
                ...
                if not song_file:
                    ydl_opts = self.ydl_opts.copy()
                    ydl_opts['outtmpl'] = os.path.join(self.download_dir, 'song.%(ext)s')  # Ensure correct path
