...
class MusicPlayer:
    def __init__(self):
        ...
        self.api_key = os.getenv('YOUTUBE_API_KEY', 'AIzaSyCuQwsagMLGToKlbNMEf7plFEkKhbr-DWs')
        ...
        self.download_thread = None  # Thread for pre-downloading
    ...
    def pre_download_first_song(self):
        ...
        def download_thread():
            try:
                ...
                logger.info(f"Pre-downloading first song: {title}")
                ...
                for ext in ["mp3", "webm", "m4a", "opus"]:
                    if os.path.exists(f"next_song.{ext}"):
                        ...
                        break
                if not found_file:
                    logger.error(f"Pre-download failed for: {title}")
            except Exception as e:
        ...
        logger.info(f"Started pre-download thread for: {title}")
    ...
    f"👤 Requested by: {requested_by}"
        ...
        # If this is the first song in the queue, pre-download it
            if queue_length == 1:
                self.pre_download_first_song()
        ...
        def play_in_thread():
            try:
                ...
                if found_file:
                    ...
                    # Rename next_song to song for playback
                    os.rename(song_file, song_file.replace("next_song", "song"))
                else:
                    ...
                    ydl_opts = {
                        ...
                        'outtmpl': 'song.%(ext)s',  # Use song for current playback
                        'quiet': True,
                        'no_warnings': True,
                        'cookiefile': 'cookies.txt'
                    }
                ...
                with self.queue_lock:
                    ...
                    self.current_duration = None
                    self.current_requested_by = None
                    # Pre-download the next song if queue is not empty
                    if self.queue:
